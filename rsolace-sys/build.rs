...
fn main() {
    ...
    if !is_docs_rs {
        // Only download and extract when not on docs.rs
        ...
        println!("cargo:warning=Skipping Solace client library download on docs.rs");
    }

    // Only set up linking when not on docs.rs
    if !is_docs_rs {
    ...
    // Only generate bindings if we have the headers available (not on docs.rs)
    if !is_docs_rs && include_path.exists() {
        ...
        println!("cargo:warning=Skipping bindgen on docs.rs - using existing bindings");
    }
